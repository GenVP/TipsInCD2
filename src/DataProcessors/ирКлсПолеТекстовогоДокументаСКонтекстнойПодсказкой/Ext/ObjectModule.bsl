...
Перем ТаблицаЛокальногоКонтекста Экспорт;
...
//КД2
Функция кдИнициализировать(
	пИмя,
	пКонфигурация,
	пФорма,
	пКонтекстВыполнения = Неопределено,
	пЛиЯзыкЗапросов = Ложь,
	пМетодВыполнения = "",
	пТипТекста = "Алгоритм") Экспорт
	
	мСообщенияЧерезПредупреждения = пФорма.МодальныйРежим;
	...
	Конфигурация.Вставить("Конфигурация", пКонфигурация);
	...
	// Проверка наличия переданного метода выполнения и определение метода выполнения и контекста выполнения
...
Процедура ВыделитьВесьТекст() Экспорт 
...
Функция ЗалитьКомментарииИСтроковыеЛитералы(Текст, СимволЗамена = " ") Экспорт 
	...
	Если СтрДлина(ОригинальныйТекст) < 10000 Тогда // Долго на больших текстах!
		...
		Если ЯзыкПрограммы = 0 Тогда
			...
			RegExp.Pattern = "(" + шЕсли + ")|(" + шПока + ")|([\s\S])";
			...
			Для Каждого Match Из Результат Цикл
				...
				//	НачалоТокена = Match.SubMatches(9);
				//	//КонецТокена = Match.SubMatches(10);
				КонецЕсли;
				...
				Иначе
					...
					ЗалитыйТекст = ЗалитыйТекст + Match.SubMatches(8);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли; 
	КонецЕсли; 
...
//
Процедура РазобратьТекущийКонтекст(ЛиСправаОтРавенства = Ложь, выхЕстьТочкаСправа = Ложь)
...
Функция ВызватьКонструкторЗапросов(ИспользуемСобственныйКонструктор = Неопределено) Экспорт
	...
	Если КонструкторЗапроса.ОткрытьМодально() = Истина Тогда
		Если Не РежимТолькоПросмотр Тогда
			...
			КонецЕсли;
			Если ЯзыкПрограммы = 1 Тогда
				Если Не ЭтоЧастичныйЗапрос И ПустаяСтрока(ПолеТекстовогоДокумента.ВыделенныйТекст) Тогда 
					ВыделитьВесьТекст();
				КонецЕсли; 
			...
			Для Каждого СтруктураТипа Из ТаблицаСтруктурТипов Цикл
				Если СтруктураТипа.СтрокаОписания <> Неопределено Тогда 
					Если ТипЗнч(СтруктураТипа.СтрокаОписания) = Тип("Структура") Тогда
						...
						СтрокаСозданияРодителя = СтруктураТипа.СтрокаОписания.СтрокаСозданияРодителя;
						Если ЗначениеЗаполнено(СтрокаСозданияРодителя) Тогда
							...
							Если Ответ = КодВозвратаДиалога.ОК Тогда
								ирОбщий.ПерейтиКОпределениюМетодаВКонфигуратореЛкс(СтрокаСозданияРодителя + "." + ирОбщий.ПолучитьСтрокуБезКонцаЛкс(мТекущееСлово, 1));
							КонецЕсли;
						КонецЕсли; 
					ИначеЕсли Истина
					...
					Тогда
						...
						Прервать;
					ИначеЕсли Истина
						И СтруктураТипа.СтрокаОписания.Владелец().Колонки.Найти("ТипЗначения") <> Неопределено 
	...
	Для Каждого РодительскаяСтрутураТипа Из МассивРодительскихСтрутурТипа Цикл
		...
		Если РодительскаяСтрутураТипа.ИмяОбщегоТипа <> "Неизвестный контекст" Тогда
			...
			ВнутренняяТаблицаСлов = мПлатформа.ПолучитьВнутреннююТаблицуМетаданныхСлов(РодительскаяСтрутураТипа,
				ТекущееСлово, ТипСлова, ВиртуальнаяТаблица, ЯзыкПрограммы, ТекущийИндекс,,, ЛиСерверныйКонтекст);
			Если ВнутренняяТаблицаСлов.Количество() > 0 Тогда
			...
			Если РезультатТекста.Count > 0 Тогда
				...
				Если Не ирОбщий.ЛиПустаяПодгруппаRegExpЛкс(ПоследнееВхождение.SubMatches(1)) Тогда
					...
					Если НРег(ПоследнееВхождение.SubMatches(3)) = Нрег("Новый") Тогда
						...
						КонецЕсли; 
						Если Ложь
							Или СтруктураТипа.ИмяОбщегоТипа = "Структура"
							Или СтруктураТипа.ИмяОбщегоТипа = "ФиксированнаяСтруктура"
						Тогда
							RegExp.Global = Ложь;
							ТекстДляЗаполненияМетаданных = Сред(ТекстДляПоискаОпределения, ПоследнееВхождение.FirstIndex + ПоследнееВхождение.Length + 1);
							ШаблонКонструкторСоСвойствами = "\s*""((?:" + шИмя + "\s*,\s*)*" + шИмя + ")""";
							...
							Если РезультатСвойств.Count > 0 Тогда
								СтруктураТипа.Метаданные = Новый Структура(РезультатСвойств.Item(0).SubMatches(0));
							Иначе
								СтруктураТипа.Метаданные = Новый Структура;
							КонецЕсли; 
							ШаблонВставкаСвойства = Контекст + "\s*\.\s*Вставить\s*\(\s*""(" + шИмя + ")""";
							RegExp.Pattern = ШаблонВставкаСвойства;
							RegExp.Global = Истина;
							РезультатСвойств = RegExp.Execute(ТекстДляЗаполненияМетаданных);
							Для Каждого ВхождениеСвойства Из РезультатСвойств Цикл
								СтруктураТипа.Метаданные.Вставить(ВхождениеСвойства.SubMatches(0));
							КонецЦикла;
						ИначеЕсли Ложь
							Или СтруктураТипа.ИмяОбщегоТипа = "ТаблицаЗначений"
							Или СтруктураТипа.ИмяОбщегоТипа = "ДеревоЗначений"
						Тогда
							RegExp.Global = Ложь;
							ТекстДляЗаполненияМетаданных = Сред(ТекстДляПоискаОпределения, ПоследнееВхождение.FirstIndex + ПоследнееВхождение.Length + 1);
							СтруктураТипа.Метаданные = Новый ТаблицаЗначений;
							ШаблонВставкаСвойства = Контекст + "\s*\.\s*Колонки\s*.\s*Добавить\s*\(\s*""(" + шИмя + ")""";
							RegExp.Pattern = ШаблонВставкаСвойства;
							RegExp.Global = Истина;
							РезультатСвойств = RegExp.Execute(ТекстДляЗаполненияМетаданных);
							Для Каждого ВхождениеСвойства Из РезультатСвойств Цикл
								СтруктураТипа.Метаданные.Колонки.Добавить(ВхождениеСвойства.SubMatches(0));
							КонецЦикла;
						КонецЕсли; 
	...
	Пока ТаблицаСтруктурТиповКонтекста.Количество() > 0 Цикл
		...
		ИначеЕсли СписокТиповКонтекста.Количество() > 1 Тогда
			...
			Для Каждого СтрокаТипа Из ВыбранныеРанееТипы Цикл
				...
				КонецЕсли; 
			КонецЦикла;
			ВыбранныйТип = СписокТиповКонтекста.ВыбратьЭлемент("Выберите тип контекста", НачальныйВыбор);
			Если ВыбранныйТип = Неопределено Тогда
				Возврат;
			КонецЕсли;
			ВыбраннаяСтруктураТипа = ВыбранныйТип.Значение;
