...
Функция ПолучитьВсеОбщиеРеквизиты(Настройки)
	...
	Возврат кдМетаданные.ПолучитьТаблицуМетаданных(Настройки, МетаданныеРодителя, "ОбщийРеквизит");
...
Функция ИмяФайлаПоМетаданным(Настройки, ПолноеИмя) Экспорт
	...
	ИначеЕсли ПустаяСтрока(МассивФрагментов[0]) Тогда
		ИмяФайла = Настройки.КорневыеТипы.Найти(МассивФрагментов[1], "Ед").ЕдEng;
	Иначе
		Разделитель = ПолучитьРазделительПути();
		ИмяФайла = Настройки.КорневыеТипы.Найти(МассивФрагментов[0], "Ед").МнEng + Разделитель + МассивФрагментов[1];
		Если МассивФрагментов.Количество() > 2 Тогда
			ИмяФайла = ИмяФайла + Разделитель + "Ext" + Разделитель + Настройки.КорневыеТипы.Найти(МассивФрагментов[2]).ЕдEng;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаполнитьМетодыПоФайлу(Настройки, ИмяФайла, ОбъектИндекса) Экспорт
	
	Разделитель = ПолучитьРазделительПути();
	
	ПолноеИмяФайла = Настройки.КаталогИсходныхФайлов + Разделитель + ИмяФайла + ".bsl";
	ТестФайл = Новый Файл(ПолноеИмяФайла);
	Если ТестФайл.Существует() Тогда
		ТекстДок = Новый ЧтениеТекста(ПолноеИмяФайла, КодировкаТекста.UTF8,,,Ложь);
		ТекстМодуля = ТекстДок.Прочитать();
		ТекстДок.Закрыть();
		Методы = ирКэш.Получить().ТаблицаМетодовМодуля(ТекстМодуля);
	Иначе
